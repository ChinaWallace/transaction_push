...
class StrategyTradingService:
    ...
    async def analyze_grid_opportunity(self, symbol: str, 
        ...
        try:
            ...
            # 获取历史数据
            klines = await self.exchange_service.get_kline_data(symbol, '1H', 200)  # 获取更多数据用于Kronos分析
            ...
            try:
                    funding_data = await self.exchange_service.get_funding_rate(symbol)
    ...
    async def analyze_dca_opportunity(self, symbol: str, 
        ...
        try:
            ...
            # 获取历史数据分析趋势
            klines = await self.exchange_service.get_kline_data(symbol, '4H', 168)  # 4小时K线，28天数据
            current_price = await self.exchange_service.get_current_price(symbol)
