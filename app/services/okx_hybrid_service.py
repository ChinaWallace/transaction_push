...
from app.services.okx_realtime_data_manager import get_realtime_data_manager, MarketData, KlineData
...
class OKXHybridService:
    ...
    async def get_funding_rate(self, symbol: str = None) -> Optional[Union[Dict[str, Any], List[Dict[str, Any]]]]:
        ...
        try:
            async with self.rest_service:
                result = await self.rest_service.get_funding_rate(symbol)
                if isinstance(result, dict):
                    result['source'] = 'rest_api'
                elif isinstance(result, list):
                    for item in result:
                        item['source'] = 'rest_api'
                return result
        except Exception as e:
    ...
    async def get_positions(self) -> List[Dict[str, Any]]:
        ...
        try:
            async with self.rest_service:
                return await self.rest_service.get_positions()
        except Exception as e:
    ...
    async def get_tickers(self, inst_type: str = 'SWAP') -> List[Dict[str, Any]]:
        """获取所有ticker数据 - 使用REST API（批量数据）"""
        try:
            async with self.rest_service:
                tickers = await self.rest_service.get_tickers(inst_type)
                for ticker in tickers:
                    ticker['source'] = 'rest_api'
                return tickers
        except Exception as e:
