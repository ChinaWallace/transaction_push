        ...
        try:
            ...
            for i in range(len(close_arr)):
                if i == 0:
                    ...
                    # 趋势延续或转换
                    if (is_uptrend[i-1] and close_arr[i] > lower_final[i]) or \
                       (not is_uptrend[i-1] and close_arr[i] > upper_final[i]):
                        supertrend[i] = lower_final[i]
                        is_uptrend[i] = True
                    else:
                        supertrend[i] = upper_final[i]
    ...
    def calculate_from_klines(self, klines: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i, kline in enumerate(klines):
                ...
                enriched_kline['supertrend_direction'] = 'up' if trend_directions[i] else 'down' if trend_directions[i] is not None else None
